(function () {
  ...
  var edges = [
    ["o0", "o1"], ["o1", "o2"], ["o2", "o3"], ["o3", "o4"], ["o4", "o5"], ["o5", "o0"],
    ["i0", "i1"], ["i1", "i2"], ["i2", "i3"], ["i3", "i4"], ["i4", "i5"], ["i5", "i0"],
    ["o0", "i0"], ["o1", "i1"], ["o2", "i2"], ["o3", "i3"], ["o4", "i4"], ["o5", "i5"],
    ["i0", "i3"], ["i1", "i4"], ["i2", "i5"]
  ];
  ...
  var nodeEls = points.map(function (p) {
    ...
    c.setAttribute("stroke", p.ring === "inner" ? "#0a86d8" : "rgba(8, 64, 103, 0.36)");
  ...
  function draw(t) {
    ...
    svg.setAttribute(
      "viewBox",
      (0 + wobbleX).toFixed(2) + " " + (0 + wobbleY).toFixed(2) + " " +
      (width - depth * 5).toFixed(2) + " " + (height - depth * 2.6).toFixed(2)
    );

    var animated = points.map(function (pt, idx) {
      var speed = pt.ring === "inner" ? 2.1 : 1.5;
      var phase = idx * 0.55;
      var x = pt.x + Math.sin(tt * speed + phase) * (pt.ring === "inner" ? 1.8 : 2.6);
      var y = pt.y + Math.cos(tt * speed * 0.95 + phase) * (pt.ring === "inner" ? 1.7 : 2.2);
      return { id: pt.id, x: x, y: y };
    });

    function a(id) {
      for (var j = 0; j < animated.length; j++) if (animated[j].id === id) return animated[j];
      return null;
    }
    ...
    for (var i = 0; i < edges.length; i++) {
      ...
      l.setAttribute("stroke-opacity", (0.25 + Math.sin(tt * 2 + i * 0.2) * 0.09).toFixed(2));
    }
